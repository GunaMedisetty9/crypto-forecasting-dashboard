...
def load_artifacts():
    ...
    return predictions_data, train_data, test_data
...
def load_price_csvs():
    ...
    btc_data = ensure_columns(btc_data)
    eth_data = ensure_columns(eth_data)

    return {"BTC-USD": btc_data, "ETH-USD": eth_data}
...
data_dict = load_price_csvs()
predictions_data, train_data, test_data = load_artifacts()
...
if show_technical:
    ...
    with tab3:
        ...
        bb_width = ((plot_data["BB_Upper"] - plot_data["BB_Lower"]) / plot_data["BB_Middle"]) * 100
        c1, c2 = st.columns(2)
        with c1:
            st.metric("Current BB Width", f"{bb_width.iloc[-1]:.2f}%")
        with c2:
